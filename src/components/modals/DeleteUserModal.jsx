...
export function DeleteUserModal({ userId, userName, onClose, onConfirm }) {
  ...
  return (
    <div id="delete-user-modal" className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg border border-gray-200">
        ...
        <div className="p-5">
          <p className="text-gray-600 mb-2">
            Are you sure you want to delete <strong className="font-bold text-gray-900">{userName}</strong>?
          </p>
          ...
          {error && <p id="delete-user-error" className="text-sm text-red-600 mb-4 p-3 bg-red-50 rounded-lg border border-red-200">{error}</p>}
        </div>
