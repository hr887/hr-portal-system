...
import { ComplianceWidget } from './admin/ComplianceWidget.jsx'; // <-- NEW IMPORT
import { LogOut, Search, ChevronDown, Users, FileText, CheckSquare, Bell, ArrowDownUp, AlertTriangle, User, ChevronUp, UserPlus, Replace, Settings, Zap } from 'lucide-react';

function StatCard({ title, value, icon }) {
  return (
    <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200 flex flex-col h-full justify-between">
...
export function CompanyAdminDashboard() {
  ...
  async function refreshData(compId) {
    ...
    setError('');
    try {
      const [appList, leadsList] = await Promise.all([
  ...
  const currentList = activeTab === 'applications' ? applications : leads;
  ...
  const qualificationCount = applications.filter(a => ['Background Check', 'Awaiting Documents'].includes(a.status)).length;
      ...
      <div id="company-admin-container" className="h-screen bg-gray-50 flex flex-col">
        ...
        <div className="container mx-auto p-4 sm:p-8 shrink-0">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
             ...
             <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div onClick={() => setActiveTab('leads')} className={`cursor-pointer transition-all ${activeTab === 'leads' ? 'ring-2 ring-purple-500 rounded-lg' : ''}`}>
                    <StatCard title="SafeHaul Leads" value={leadsCount} icon={<Zap size={20}/>} />
                </div>
                <div onClick={() => setActiveTab('applications')} className={`cursor-pointer transition-all ${activeTab === 'applications' ? 'ring-2 ring-blue-500 rounded-lg' : ''}`}>
                    <StatCard title="Total Applicants" value={appCount} icon={<FileText size={20}/>} />
                </div>
                <StatCard title="In Progress" value={qualificationCount} icon={<CheckSquare size={20}/>} />
             </div>
             
             {/* --- Right Side: Compliance Widget --- */}
             <div className="lg:col-span-4">
                {/* Pass ALL applications so it can check approvals and dates */}
                <ComplianceWidget applications={applications} />
             </div>
          </div>
        </div>
        ...
        <main className="container mx-auto px-4 sm:px-8 pb-8 grid grid-cols-1 lg:grid-cols-12 gap-8 overflow-hidden flex-1 min-h-0">
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden flex-1 flex flex-col h-full">
              <div className="p-5 border-b border-gray-200 bg-gray-50">
                <h2 className="text-xl font-semibold text-gray-800">
                    {activeTab === 'leads' ? 'Platform Leads' : 'Applications'}
                </h2>
                <p className="text-xs text-gray-500 mt-1">
                    {activeTab === 'leads' ? 'Drivers interested in your company.' : 'Drivers who completed full application.'}
                </p>
              </div>

              <div className="p-5 flex flex-col gap-4 border-b border-gray-200 shrink-0">
                <div className="relative w-full">
                  <input type="text" placeholder="Search..." className="w-full p-3 pl-10 border border-gray-300 rounded-lg" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              ...
              <div className="overflow-y-auto flex-1 min-h-0">
                <table className="w-full">
                  <tbody className="divide-y divide-gray-200">
                    ...
                    {!loading && filteredList.length === 0 && <tr><td className="p-5 text-center text-gray-500">No records found.</td></tr>}
                    {!loading && filteredList.map(item => {
                      ...
                      const isLead = !!item.isPlatformLead;
                          ...
                          <td className="px-5 py-4">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500">{getFieldValue(item.phone)}</p>
                                </div>
                                {isLead && <span className="bg-purple-100 text-purple-700 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider">Lead</span>}
                            </div>
          ...
          <div className="lg:col-span-7 xl:col-span-8 min-h-0">
            {selectedApp ? (
               selectedApp.isPlatformLead ? (
                  <div className="bg-white rounded-xl shadow-lg border border-gray-200 h-full p-8 overflow-y-auto">
                      <div className="flex items-center gap-3 mb-6">
                          <div className="p-3 bg-purple-100 text-purple-600 rounded-full"><Zap size={32} /></div>
                          <div>
                             <h2 className="text-2xl font-bold text-gray-900">SafeHaul Lead</h2>
                             <p className="text-gray-500">Applicant showed interest via 1-Click Apply</p>
                          </div>
                      </div>
                      <div className="grid grid-cols-2 gap-6 mb-8">
                          ...
                          <div><label className="text-xs font-bold text-gray-500 uppercase">Phone</label><p className="text-lg">{selectedApp.phone}</p></div>
                          <div><label className="text-xs font-bold text-gray-500 uppercase">Email</label><p className="text-lg">{selectedApp.email}</p></div>
                          <div><label className="text-xs font-bold text-gray-500 uppercase">Experience</label><p className="text-lg">{selectedApp.experience}</p></div>
                          <div><label className="text-xs font-bold text-gray-500 uppercase">Driver Type</label><p className="text-lg">{selectedApp.driverType}</p></div>
                      </div>
                      <div className="bg-blue-50 border border-blue-100 rounded-lg p-6 text-center">
                          <p className="text-blue-800 font-medium mb-4">Contact this driver to complete the full application.</p>
                          <button className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-md transition-all">Send Invite (Simulated)</button>
                      </div>
                  </div>
               ) : (
