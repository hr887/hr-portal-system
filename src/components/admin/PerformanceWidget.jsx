  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // 2. Fetch Real Names & Initialize Stats
            const memberDataPromises = members.map(async (member) => {
                ...
                return {
                    uid,
                    name,
                    dials: 0,      // Total attempts
                    contacts: 0,   // Successful connections
                    stats: {}      // Detailed outcome counts for the modal
                };
            });
            ...
            activitiesSnap.forEach(docSnap => {
                ...
                if (statsMap[uid]) {
                    ...
                    // Collect detailed outcomes for the modal report
                    if (data.outcome) {
                        const key = data.outcome;
                        statsMap[uid].stats[key] = (statsMap[uid].stats[key] || 0) + 1;
                    }
                }
            });

            // 5. Sort (Contacts Desc -> Dials Desc)
            const lb = Object.values(statsMap).sort((a, b) => {
  ...
  return (
    <>
        <div 
            ...
            className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 h-40 flex flex-col relative overflow-hidden cursor-pointer hover:border-blue-300 hover:shadow-md transition-all"
